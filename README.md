...
“声音” 子设备中的 **扬声器** 支持 HA 媒体浏览、音频 URL、`media_player.play_media` 和 `tts.speak`。HA 使用 FFmpeg 将音频流式转换为 PCM，经 LAN 推送到 P3 自带播放器。音频在 P3 上通过内存管道播放；播放会话结束后释放端口与播放器进程。

TTS／媒体源按 HA 标准接口解析为 URL，由 HA 自己获取音频，再主动连接 P3 推送 PCM。HA 自身提供的音频优先通过实际本机 HTTP 监听地址读取，保留原有路径签名；第三方 URL 使用原地址。HA 原生启用 HTTPS 时使用其配置的地址并验证证书。

- HA OS 可直接使用；其他安装方式需要 HA 能运行 FFmpeg。
- 支持 FFmpeg 可解码的单个 HTTP/HTTPS 音频流，如 WAV、MP3。播放列表、HLS、跳转进度与暂停续播暂未实现。
- 媒体音量默认为 20%，可实时调整或静音，HA 重启后恢复设置；它独立于原厂音效音量。
- 新媒体、TTS 和本集成的内置音效会互相替换。语音播报结束后进入空闲；原厂自主触发的提示音仍可能争用同一声卡。
- 当前要求通过 IPv4 访问 P3，并允许 HA 连接设备在播放期间分配的临时 TCP 端口。端口校验本次会话的随机凭据，音频通过 LAN 明文传输。
- 每次播放上限一小时；断流超时或取消时回收播放进程。设备端缓冲有界，不随音频长度增长。

纯本地语音可选择 HA 中的本地 TTS 引擎。HA 自身的 TTS 缓存由其配置管理。
...
TTS 示例：在自动化编辑器选择自己的 TTS 引擎和 P3 扬声器，或替换以下实体 ID：
...
data:
  media_player_entity_id: media_player.YOUR_P3_SPEAKER
  message: "空调已开启"
  cache: false
```

媒体播放器的停止操作结束当前媒体；“停止音效”按钮和 `aqara_p3.stop_sound` 同时停止本集成的媒体和内置音效。
...
- 随组件提供小型静态 MIPS helper，按需复制至设备 `/tmp`，校验设备身份、固件及文件哈希后运行。电量读取通过原厂 ELF 符号表定位变量，结合进程实际加载位置只读积分计数；符号缺失或有歧义时停止读取。累计电量的结构体成员偏移仍由已验证的 4.0.4 固件布局限定。原生源码和构建脚本随仓库提供。
- 媒体播放使用独立临时 Telnet 会话和鉴权 TCP 音频连接，复用原厂 `aplay`，以显式结束帧区分播完与连接中断。原厂播放器指纹不匹配时拒绝启动。HA 负责转码与软件音量，不影响空调控制连接。
...
测试使用回环地址、虚构设备标识和协议测试向量，可离线运行，不需要实机或固件镜像。测试覆盖 HA 配置流程、实体与动作、协议编解码、模式记忆、定时状态、红外抓取生命周期、音效和媒体播放任务。原生进程测试需要 C 编译器，实际解码测试需要测试主机安装 FFmpeg。2026-09-19 已实机验证串流语音、提前停止、替换播放和内置音效切换；不同设备的声音效果及部分空调特殊功能仍需核对。
